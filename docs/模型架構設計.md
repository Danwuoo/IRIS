...
## 1) 整體權重拓撲：Mamba Trunk 作為唯一大容量，Level Heads 以 Adapter/Head 形式掛載
...
- **(\Theta_{\text{mamba_trunk}})**：主幹 SSM/Mamba 層（唯一的「大網路」）。
- **(\Theta_{k})**：Level k 的 head / operator / adapter 權重（每層必須存在）。
...
> SSM/Mamba trunk 的使用方式，就是把這個 (Z) 當作“序列”處理；但是要加上 type embedding（object/relation/event/…），否則 Mamba 無法區分 token 類型。
> 

---

## 3) Mamba/SSM Trunk 具體組合方式

### 3.1 Trunk 結構（建議：多層 Mamba block + 小型 cross-token mixing）

純 Mamba 對「token 類型交互」不如注意力直觀，因此建議做一個**最小混合**，仍以 SSM 為主：
...
目標：保持 SSM 的效率優勢，但仍能讓 object/relation/event 在同層互相影響。

### 3.2 位置/時間處理

- 對 ARC 這種靜態輸入，你需要的是「結構位置」，不是時間序列。
- 對 ARC-AGI-3 互動與 agent 迭代，你需要 event-time encoding。
...
> 若 ARC-AGI-2 靜態題，L1 可以弱化到「可選 rollout」；但權重模組仍需存在（可被 controller 置零使用）。
> 

---

### Level 2：程序歸納（權重必備，且不能是純手寫執行器）

這層是 ARC/數學/agent 的核心，你需要「proposal + executor + scorer」。
...
- Output：K 個 program 候選（program tokens / sketch）
...
- Input：program + (Z)
- Output：更新後的 (Z) 或輸出候選解/子目標 token
- 建議做法：primitive 是 learned operator（例如對 objects 的 soft select、對 relations 的 learned match、對 transforms 的 parameter head），控制流可先硬。
...
### Level 4：Program / Concept Memory（權重必備）
...
### Level 5：Abstraction Management（權重必備）
...
> MVP 階段可以把 macro 粒度固定，但 selector/updater 的權重仍要存在（可學 gate 可先偏向固定）。
> 

---

### Level 6：Self-Monitoring / Meta-Credit（權重必備）
...
- Input：失敗/低信心時的診斷特徵
- Output：對 Level 0/1/2/4 的調用偏好（例如：重跑感知、加深 rollout、擴大 program beam、增加檢索 k）
...
- 把 memory read、program execution、rollout 結果融合回 (G)/(T)/部分 object tokens（gated residual / FiLM）
...
- **Trunk**：L 層 Mamba blocks（你可從 8–16 層起跳做 MVP）
...
每一步都有對應權重模組；即便某些步驟在某次迭代被 gate 掉，模組仍存在於權重集合內，符合你的約束。
...
2. **mamba_trunk/**：block_0 … block_{L-1}（含 mixer、norm）
...
8. **level5/**：macro_selector、macro_updater（subprogram_extractor 可選）
...
| Token 類型 | 建議上限 | 說明 |
| --- | --- | --- |
| Object (N_o) | **32** | ARC 類任務幾乎夠用；agent/數學可先 objectize 成有限集合 |
